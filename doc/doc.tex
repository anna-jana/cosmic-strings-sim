...
\subsubsection{Moore}
\subsubsection{Hiramatsu}
\subsubsection{Gorghetto}
...
\begin{figure}[H]
	\includegraphics[width=\textwidth]{../string_solution.pdf}
...
The solution for a single string has an associated energy per unit length which we obtain by integrating out the radial and angular directions.
The dominant contribution is coming from the gradient energy of the angular (axion) field.

\begin{equation}
	\mu = \int \rho \mathrm{d} \rho \int \mathrm{d} \alpha ( V(\phi) + |\nabla \phi|^2 )
\end{equation}
