...
def compute_power_spectrum(p:AxionStrings.Parameter, field, spheres, surface_element, bin_ks):
    ...
    for i in range(p.nbins):
        for (ix, iy, iz) in spheres[i]:
            spectrum[i] += np.abs2(field_fft[ix, iy, iz])
        ...
        for j in range(p.nbins):
            ...
            for idx1 in spheres[i]:
                for idx2 in spheres[j]:
                    s += np.abs2(substract_wave_numbers_lookup(
...
def compute_spectrum_autoscreen(p : AxionStrings.Parameter, s : AxionStrings.State):
    ...
    for i in range(p.nbins):
        for (ix, iy, iz) in my_sphere_parts[i]:
            spectrum[i] += np.abs2(screened_theta_dot_fft_global[ix, iy, iz])

        MPI.Reduce(spectrum[i], operator.add, s.root, s.comm)
